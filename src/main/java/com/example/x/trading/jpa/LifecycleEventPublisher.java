package com.example.x.trading.jpa;

import com.example.x.trading.jpa.core.AggregateRoot;
import com.example.x.trading.jpa.event.LifecycleEvent;
    ...
    private Set<? extends AggregateRoot> extract(Object[] objects) {
        ...
        Stream.of(objects).forEach(o -> {
            if (ClassUtils.isAssignable(Collection.class, o.getClass())) {
                ...
                c.stream().filter(a -> ClassUtils.isAssignable(AggregateRoot.class, a.getClass())).forEach(a -> aggregates.add((AggregateRoot) a));
