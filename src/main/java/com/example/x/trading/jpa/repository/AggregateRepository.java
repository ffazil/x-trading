package com.example.x.trading.jpa.repository;
...
public interface AggregateRepository<T extends AggregateRoot> extends JpaRepository<T, UUID> {
    ...
    Optional<T> findInactiveById(UUID id);
    ...
    default boolean existsById(UUID id) {
        return findById(id).map(t -> t.isActive()).orElse(false);
    }
    ...
    default void purgeAll(Iterable<T> entities) {
        List<UUID> ids = StreamSupport.stream(entities.spliterator(), false).map(entity -> entity.getId()).collect(Collectors.toList());
